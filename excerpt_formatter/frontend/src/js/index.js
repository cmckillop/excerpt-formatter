...
async function postMessageRequest() {
  const postData = {
    ...
    word_count: parseInt(
      document.getElementById("tag-word-count").dataset.wordCount
    ),
  };
...
async function postFormatRequest() {
  ...
  const programmeId = document.getElementById("input-programme-id").value;
  const audienceIndex = document.getElementById("input-audience-index").value;
  ...
  if (offsetTime !== "") {
    postData.offset_time = offsetTime;
  }

  if (programmeId !== "") {
    postData.programme_id = programmeId;
  }

  if (audienceIndex !== "") {
    postData.audience_idx = audienceIndex;
  }
...
document
  .getElementById("button-clear-offset-time")
  .addEventListener("click", function () {
    document.getElementById("input-offset-time").value = "";
  });

document
  .getElementById("button-auth-token")
  .addEventListener("click", function () {
    getAuthenticationRequest().then((data) => {
      const inputAuthToken = document.getElementById("input-auth-token");
      const authTokenStatusIcon = document.getElementById(
        "auth-token-status-icon"
      );
      if (data["ok"]) {
...
document
  .getElementById("button-channel-id")
  .addEventListener("click", function () {
    const inputChannelId = document.getElementById("input-channel-id");
    const channelIdStatusIcon = document.getElementById(
      "channel-id-status-icon"
    );
